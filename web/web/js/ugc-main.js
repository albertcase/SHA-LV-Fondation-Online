		...
		_doing = {
			...
			dreamListData : function(){     //index
				$("#dreamListCon").html(
					$.map(this.basedata,function(k){
						return '<div class="swiper-slide"><div class="listmodel m-1"><div class="d_content"><p class="d-to">234'+k+'<sup>th</sup></p><p class="d-text">辛苦努力创作的作品终于被大家认可。下一步要在美术馆里开个人展</p><p class="d-from">Michel</p></div><img src="../imgs/ugc/listmodel.png" width="100%" /></div></div> '
					}).join("")
				);
			...
			getGlassData : function(){  //chose
				$.ajax({
					...
					$("#glassListCon").html($.map(data,function(v,k){
						return '<div class="swiper-slide"><div class="gl g-'+parseInt(k+1)+'""><a href="javascript:;"><img src="../imgs/ugc/g-'+parseInt(k+1)+'.png" width="100%" /></a></div><img src="'+v.pic_thumbnail+'" width="100%" /></div>'
					}).join(""));
					...
					var glassSwiper = $('.glass-swiper').swiper({
					        ...
					        onSlideChangeEnd: function(swiper){
						      	$("#cur-glass").attr("src","../imgs/ugc/glass-bg-"+parseInt(swiper.activeIndex+1)+".jpg")
						    }
					});
			...
			getSelectedData : function(){    //selected
				$("#selectedContent").html(
					'<div class="selectedModel_con"><div class="selectedNum">2034<sup>th</sup></div><div class="selectedCon">辛苦努力创作的作品终于被大家认可。下一步要在美术馆里开个人展</div><div class="selectedName">Michel</div></div><img src="../imgs/ugc/d-m-bg.png" width="100%" />'
				);
			},
